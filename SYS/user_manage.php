...
$connection = conn();
$stmt = $connection->prepare("SELECT  id, user_status  FROM  rrb_users where login = ? and pass = ?;");
...
function login_log($p_userid) {
    ...
    $user_id = $p_userid;
     ...
     $mysqli_conn = conn();  
            $stmt_i = $mysqli_conn->prepare("INSERT INTO rrb_user_login ".
...
function login_check($login_id,  $user_ip){
 $sql_check =  " SELECT user_id as c FROM rrb_user_login ".
               "  where  id = ?                      ".
               "  and login_status = 1               ".
               "  and user_ip = ?          ";
